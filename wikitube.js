...
const Wikitube = (function(){
	...
	function getApiKey() {
		...
		if (!key) {
			key = prompt('[Wikitube] YouTube API key not set. Please enter your YouTube Data API v3 key:', '');
			if (key) {
	...
	function isAllowedPath(path) {
		...
		if (onWikipediaIndex) {
			...
			const prefixes = ['/wiki/', ...wikipedia_lang_codes.map(function(lang){ return '/' + lang + '/'; })];
		...
		for (let i = 0; i < banned_title_prefixes.length; i++) {
			if (articleTitle.indexOf(banned_title_prefixes[i]) === 0) { return false; }
		}
		for (let j = 0; j < banned_titles.length; j++) {
			if (articleTitle === banned_titles[j]) { return false; }
		}
		return true;
	}
	...
	function determineContext() {
		if ($('#mw-content-text').length) {
			...
			return {
				...
				numVideosToLoad: Math.floor($('.body-wrapper .content-wrapper .content').width() / 350) + 1,
				insertBefore: '.main-content'
			};
		} else if ($('#fullContent').length) {
			return {
				titleText: $('.firstHeading > span').text(),
				numVideosToLoad: Math.floor($('#article_content_wrapper').width() / 350),
	...
	function setHorizScroll(){
		$('#wikitube_container').on('mousewheel DOMMouseScroll', function(e){
			let delt = null;
			if (e.type == 'mousewheel') { delt = (e.originalEvent.wheelDelta * -1); }
			else if (e.type == 'DOMMouseScroll') { delt = 40 * e.originalEvent.detail; }
			if (delt) { e.preventDefault(); $(this).scrollLeft(delt + $(this).scrollLeft()); }
		});
	}

	function setupContainer(insertBeforeSelector){
		state.container = $('<div id="wikitube_container"></div>');
		state.moreButton = $('<div class="plusBtn" title="Load more videos!"></div>');
		...
		state.moreButton.click(function(){ loadAndRender(false); });
		$('iframe').ready(function(){ setHorizScroll(); });
	}
	...
	function loadAndRender(isFirstLoad){
		...
		function processVideos(videoItems){
			if (isFirstLoad) { setupContainer(context.insertBefore); }
		...
		if (cachedItems) { processVideos(cachedItems); return; }
		const url = 'https://www.googleapis.com/youtube/v3/search?part=snippet&q=' + encodeURIComponent(state.titleText) + '&key=' + state.apiKey + '&maxResults=' + (state.numVideosLoaded + state.numVideosToLoad);
