...
(function () {
  ...
  function getUsageData() {
    ...
    if (!usageElement) {
      debugPrint(
        "error",
        "Usage element not found with selector:",
        USAGE_ELEMENT_SELECTOR
      );
      return null;
    }
    ...
    if (!valueSpan || !totalSpan) {
      debugPrint(
        "error",
        "Usage value or total span not found within usageElement:",
    ...
    const totalQuota = parseFloat(
      totalSpan.textContent.trim().replace("/", "")
    );

    if (isNaN(currentUsage) || isNaN(totalQuota) || totalQuota === 0) {
      debugPrint(
        "error",
        "Could not parse usage data. Current:",
        valueSpan.textContent,
        "Total:",
        totalSpan.textContent
      );
    ...
    debugPrint("info", "Successfully parsed usage data:", {
      currentUsage,
      totalQuota,
      usagePercentage,
    });
  ...
  function getProratedUsageInfo() {
    ...
    const cycleStartDateInUTC8 = new Date(
      cycleStartYear,
      cycleStartMonth,
      RESET_DAY_OF_MONTH,
      0,
    ...
    const cycleStartEpochUTC =
      Date.UTC(cycleStartYear, cycleStartMonth, RESET_DAY_OF_MONTH, 0, 0, 0) -
      8 * 60 * 60 * 1000;
    ...
    const cycleEndEpochUTC =
      Date.UTC(cycleEndYear, cycleEndMonth, RESET_DAY_OF_MONTH, 0, 0, 0) -
      8 * 60 * 60 * 1000 -
      1; // minus 1 millisecond
    ...
    if (timePassedInCycle < 0) {
      debugPrint("error", "Time passed is negative. Check cycle date logic.", {
        now: now.toISOString(),
        cycleStartUTC: cycleStartDateUTC.toISOString(),
        cycleEndUTC: cycleEndDateUTC.toISOString(),
        nowInUTC8_debug: nowInUTC8.toString(),
        cycleStartDateInUTC8_debug: cycleStartDateInUTC8.toString(),
      });
    ...
    if (totalTimeInCycle <= 0) {
      debugPrint(
        "error",
        "Total time in cycle is not positive. Check cycle date logic.",
        {
          cycleStartUTC: cycleStartDateUTC.toISOString(),
          cycleEndUTC: cycleEndDateUTC.toISOString(),
        }
  ...
  function updateUsageBarColor() {
    ...
    debugPrint(
      "info",
      `Actual usage: ${(actualUsageRatio * 100).toFixed(1)}%, Prorated: ${(
        proratedRatio * 100
      ).toFixed(1)}%`
    );
    ...
    if (!barElementContainer) {
      debugPrint(
        "error",
        "Usage bar container not found with selector:",
        USAGE_BAR_SELECTOR
      );
      return;
    }
    ...
    if (!barElement || !(barElement instanceof HTMLElement)) {
      debugPrint(
        "error",
        "Actual usage bar element (firstChild of container) not found or not an HTMLElement.",
        barElementContainer.innerHTML
      );
      return;
    }
    ...
    if (indicator.style.left !== indicatorPosition) {
      ...
      debugPrint(
        "info",
        "Updated prorated usage indicator position to:",
        indicatorPosition
      );
    }
    ...
    const defaultBarColor = isDarkMode
      ? DEFAULT_BAR_COLOR_DARK
      : DEFAULT_BAR_COLOR_LIGHT;
    debugPrint(
      "info",
      `Theme: ${
        isDarkMode ? "Dark" : "Light"
      }. Default bar color: ${defaultBarColor}`
    );
    ...
    if (difference > 0.1) {
      ...
      debugPrint(
        "info",
        `Usage is >10% over prorated (Difference: ${(difference * 100).toFixed(
          1
        )}%). Setting color to RED: ${COLOR_RED}`
      );
    } else if (difference > 0.05) {
      newColor = COLOR_YELLOW;
      debugPrint(
        "info",
        `Usage is >5% over prorated (Difference: ${(difference * 100).toFixed(
          1
        )}%). Setting color to YELLOW: ${COLOR_YELLOW}`
      );
    } else {
      ...
      debugPrint(
        "info",
        `Usage is within 5% of prorated or less (Difference: ${(
          difference * 100
        ).toFixed(1)}%). Setting/Keeping default color: ${defaultBarColor}`
      );
    }
    ...
    if (barElement.style.backgroundColor !== newColor) {
      ...
      debugPrint(
        "info",
        "Bar color is already correct, no change needed.",
        newColor
      );
    }
    ...
    if (backgroundBarElement && backgroundBarElement instanceof HTMLElement) {
      ...
      if (
        ...
        debugPrint(
          "info",
          "Background bar style updated. Color:",
          newColor,
          "Opacity:",
          newOpacity
        );
      ...
      debugPrint(
        "warn",
        "Background bar element not found or not an HTMLElement.",
        backgroundBarElement
      );
    }
  }
  ...
  function observeDOMChanges() {
    ...
    if (commonAncestor) {
      const observer = new MutationObserver((mutationsList) => {
        for (const mutation of mutationsList) {
          ...
          if (
            ...
            if (
              (usageEl &&
                (mutation.target.contains(usageEl) ||
                  mutation.target === usageEl ||
                  usageEl.contains(mutation.target))) ||
              (barEl &&
                (mutation.target.contains(barEl) ||
                  mutation.target === barEl ||
                  barEl.contains(mutation.target))) ||
              (usageEl &&
                mutation.target.nodeType === Node.TEXT_NODE &&
                usageEl.contains(mutation.target.parentElement))
            ) {
              relevantChange = true;
            }
          }
          if (relevantChange) {
            debugPrint(
              "info",
              "Relevant DOM change detected via MutationObserver. Re-evaluating usage bar color.",
              mutation
            );
            updateUsageBarColor();
      ...
      observer.observe(commonAncestor, {
        childList: true,
        subtree: true,
        characterData: true,
      });
      debugPrint(
        "info",
        "MutationObserver successfully set up on:",
        commonAncestor
      );
    } else {
      debugPrint(
        "warn",
        "Could not find common ancestor for MutationObserver. Falling back to interval check for updates."
      );
      setInterval(() => {
  ...
  function init() {
    if (hasInitialized) {
      debugPrint(
        "info",
        "Initialization already attempted/completed. Skipping further init calls."
      );
      return;
    }
    hasInitialized = true;
    ...
    debugPrint(
      "info",
      `Script v0.4 initializing... Path: ${window.location.pathname}`
    );
    ...
    const intervalId = setInterval(() => {
      ...
      debugPrint(
        "info",
        `Attempting to find elements (attempt ${attempts}/${maxAttempts})...`
      );
      const usageElement = document.querySelector(USAGE_ELEMENT_SELECTOR);
      const barElementContainer = document.querySelector(USAGE_BAR_SELECTOR);
      ...
      if (
        usageElement &&
        barElementContainer &&
        barElementContainer.firstChild
      ) {
        ...
        if (!barElementContainer)
          debugPrint("warn", "Bar element container not yet found.");
        else if (!barElementContainer.firstChild)
          debugPrint(
            "warn",
            "Bar element container found, but its firstChild (the bar itself) is missing."
          );
        ...
        if (attempts >= maxAttempts) {
          ...
          debugPrint(
            "error",
            "Elements not found after maximum attempts. Script will not run."
          );
        }
      }
    }, 500);
  ...
  debugPrint(
    "info",
    "Script execution started (v0.4). Checking document.readyState...",
    document.readyState
  );
  if (
    ...
    debugPrint(
      "info",
      "DOM not yet complete. Adding DOMContentLoaded listener."
    );
